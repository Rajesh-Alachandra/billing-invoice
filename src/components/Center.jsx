...
function Center() {
  ...
  const currentProfiles = invoices.slice(
  ...
  const totalPages = Math.ceil(invoices.length / propertiesPerPage);
  const maxVisiblePages = 5;

  // Add ellipsis to the pagination if necessary
  const addEllipsis = (start, end) => {
    if (end - start > 1) {
      paginationItems.push(<span key={`ellipsis-${start}-${end}`}>...</span>);
    }
  };
  ...
  for (let number = 1; number <= totalPages; number++) {
    ...
    if (isInRange) {
      paginationItems.push(
        <button
          key={number}
          className={`px-3 pt-1 rounded-md focus:outline-none ${
            isCurrent
      ...
      addEllipsis(paginationItems.length, number);
    }
  }
      ...
      <div className="dark:bg-[#141625] scrollbar-hide duration-300 min-h-screen bg-[#f8f8fb] py-[34px] px-2 md:px-8 lg:px-12 lg:py-[72px]  ">
        ...
        >
          ...
          <div className=" mt-10   space-y-4">
            {currentProfiles.map((invoice, index) => (
              <motion.div
                key={invoice.id}
